...
对全项目执行 `tsc --noEmit` 后，过滤本批目标文件的输出（`app/api/admin/scripts/[id]/route.ts`、`admin-scripts-update-delete`），未发现任何类型错误。

## 备注

- 实际测试文件命名为 `admin-scripts-update-delete.test.ts`（任务描述中的 `admin-scripts-detail.test.ts` 为类似命名）。
